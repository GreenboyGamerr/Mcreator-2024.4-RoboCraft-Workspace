...
import net.mcreator.mechafront.procedures.TNTCannonProjectileHitsBlockProcedure;
import net.mcreator.mechafront.init.MechafrontModEntities;

import javax.annotation.Nullable;

@OnlyIn(value = Dist.CLIENT, _interface = ItemSupplier.class)
public class TNTCannonEntity extends AbstractArrow implements ItemSupplier {
	public static final ItemStack PROJECTILE_ITEM = new ItemStack(Blocks.TNT);
	private int knockback = 0;

	public TNTCannonEntity(EntityType<? extends TNTCannonEntity> type, Level world) {
		super(type, world);
	}

	public TNTCannonEntity(EntityType<? extends TNTCannonEntity> type, double x, double y, double z, Level world, @Nullable ItemStack firedFromWeapon) {
		super(type, x, y, z, world, PROJECTILE_ITEM, firedFromWeapon);
	}

	public TNTCannonEntity(EntityType<? extends TNTCannonEntity> type, LivingEntity entity, Level world, @Nullable ItemStack firedFromWeapon) {
	...
	public void playerTouch(Player entity) {
		...
		TNTCannonProjectileHitsBlockProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	}
	...
	public void onHitBlock(BlockHitResult blockHitResult) {
		...
		TNTCannonProjectileHitsBlockProcedure.execute(this.level(), blockHitResult.getBlockPos().getX(), blockHitResult.getBlockPos().getY(), blockHitResult.getBlockPos().getZ());
	}
	...
	public static TNTCannonEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		TNTCannonEntity entityarrow = new TNTCannonEntity(MechafrontModEntities.TNT_CANNON.get(), entity, world, null);
	...
	public static TNTCannonEntity shoot(LivingEntity entity, LivingEntity target) {
		TNTCannonEntity entityarrow = new TNTCannonEntity(MechafrontModEntities.TNT_CANNON.get(), entity, entity.level(), null);
