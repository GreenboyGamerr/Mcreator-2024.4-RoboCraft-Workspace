...
public class ItemAnimationFactory {
	...
	public static void animatedItems(PlayerTickEvent.Post event) {
		...
		if (mainhandItem.getItem() instanceof GeoItem || offhandItem.getItem() instanceof GeoItem) {
			if (mainhandItem.getItem() instanceof RoboSwordItem animatable) {
				...
				if (!animation.isEmpty()) {
					...
					if (event.getEntity().level().isClientSide()) {
						((RoboSwordItem) event.getEntity().getMainHandItem().getItem()).animationprocedure = animation;
					}
				}
			}
			if (offhandItem.getItem() instanceof RoboSwordItem animatable) {
				...
				if (!animation.isEmpty()) {
					...
					if (event.getEntity().level().isClientSide()) {
						((RoboSwordItem) event.getEntity().getOffhandItem().getItem()).animationprocedure = animation;
					}
				}
			}
		}
